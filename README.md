...
- отдельный раздел аккаунта со статусом, продлением, покупкой и выбором «подписка уже есть / подписки нет»;
- отображение аккаунта, активности и срока подписки с автоматической синхронизацией профиля;
- PXL Guard: переключение на доступный сервер при отказе выбранного узла;
...
- переключатель полного туннеля, при котором локальная сеть остаётся доступной;
