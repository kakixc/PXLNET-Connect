...
class MainActivity :
    ...
    fun SFAApp() {
        ...
        val groupsViewModel: GroupsViewModel? =
            if (isGroupsRoute || isDashboardRoute) {
                viewModel(
        ...
        val visibleBottomNavigationScreens =
            if (developerMode) {
                bottomNavigationScreens
            } else {
                bottomNavigationScreens.filter {
                    it == Screen.Dashboard || it == Screen.Account || it == Screen.Settings
                }
            }
                    ...
                    bottomBar = {
                        if (!isSubScreen) {
                            ...
                            NavigationBar(
                                ...
                                visibleBottomNavigationScreens.forEach { screen ->
                                    NavigationBarItem(
                                        icon = {
                                            if (screen == Screen.Settings && hasUpdate) {
                                                BadgedBox(badge = { Badge(containerColor = MaterialTheme.colorScheme.primary) }) {
